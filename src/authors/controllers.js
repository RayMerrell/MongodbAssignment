...
async function getAuthor(fn, ln) {
  const existingAuthor = await Author.findOne({ firstName: fn, lastName: ln });
  if (existingAuthor) {
...
const addAuthor = async (req, res) => {
  try {
    const existingAuthor = await getAuthor(
      req.body.firstName,
      req.body.lastName
    );
    if (!existingAuthor) {
...
const updateBookAuthor = async (req, res) => {
  try {
    const foundBook = await Book.findOne({ title: req.body.title }).exec();
